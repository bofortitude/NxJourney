...
class TelnetCommandLine:
    ...
    def login(self):
        ...
        read_content = self.telnet_obj.read_until(six.b('login: '))
        self.record(read_content.rstrip())
        self.telnet_obj.write(six.b(self.username + '\n'))
        read_content = self.telnet_obj.read_until(six.b('Password: '))
        self.record(read_content.rstrip())
        self.telnet_obj.write(six.b(self.passwd + '\n'))
        read_content = self.telnet_obj.read_until(six.b(self.expect_string), timeout=10)
        self.record(read_content.rstrip())

    def run_cmd(self, command):
        try:
            self.telnet_obj.write(six.b(str(command) + '\n'))
            self.record(self.telnet_obj.read_until(six.b(self.expect_string), timeout=10).rstrip())
        except:
            self.record('', linebreak=True)
            self.record('Something wrong while running the command "' + str(command) + '"!!!')
            # raise StandardError
            raise Exception
    ...
    def return_root_view(self):
        self.telnet_obj.write(six.b('\n'))
        read_content = self.telnet_obj.read_until(six.b(self.expect_string), timeout=10).rstrip()
        self.record(read_content)
        if read_content.find(') ' + self.expect_string.rstrip()) == -1:
            return
        for i in range(10):
            self.telnet_obj.write(six.b('\x03'))  # send Ctrl-c combo
            read_content = self.telnet_obj.read_until(six.b(self.expect_string), timeout=10).rstrip()
            self.record(read_content)
            if read_content.find(') ' + self.expect_string.rstrip()) == -1:
                return
...
class TelCmdLine():
    ...
    def run_cmd(self, command):
        try:
            self.telnet_obj.write(six.b(command + '\n'))
            receivedMsg = self.telnet_obj.read_until(six.b(self.expect_string), timeout=10).rstrip()
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        six.print_('Usage:')
        six.print_('')
        six.print_('./TelnetCommandLine.py <ADC_IP> <CMD_File>')
        six.print_('')
        exit()
