...
class ip_net():
    ...
    def int_to_ip(self, number, format='ipv4'):
        ...
        if format == 'ipv4':
            ...
            return str(int(ip_address[0:8], 2)) + '.' + str(int(ip_address[8:16], 2)) + '.' + str(
                int(ip_address[16:24], 2)) + '.' + str(int(ip_address[24:32], 2))
        elif format == 'ipv6':
            ip_address = bin(number).replace('0b', '')
    ...
    def ip_random(self, ip_prefix, count):
        ...
        if ip_prefix.find(':') == -1:
            ...
            for infor in xrange(count):
                random_ip_list.append(self.int_to_ip(random.randint(self.ip_to_int(self.ip_subnet(ip_prefix)[0]),
                                                                    self.ip_to_int(self.ip_subnet(ip_prefix)[1]))))
        else:
            # IPv6
            for infor in xrange(count):
                random_ip_list.append(self.int_to_ip(random.randint(self.ip_to_int(self.ip_subnet(ip_prefix)[0]),
    ...
    def if_ip_in_iprange(self, ip_address, ip_range):
        ...
        for infor in ip_range_list:
            if infor.find('-') == -1:
                ...
                if ip_address_int <= max(first_ip_int, second_ip_int) and ip_address_int >= min(first_ip_int,
                                                                                                second_ip_int):
                    if_in_range = True
                    break
        return if_in_range
    ...
    def ip_subnet(self, ip_prefix):
        ...
        if ip_string.find(':') == -1:
            ...
            return self.int_to_ip(int(''.join(ip_address_list)[0:prefix_num] + '0' * zero_count, 2)), self.int_to_ip(
                int(''.join(ip_address_list)[0:prefix_num] + '1' * zero_count, 2))
        else:
            # IPv6
            ...
            return self.int_to_ip(int(''.join(ip_address_list)[0:prefix_num] + '0' * zero_count, 2),
                                  format='ipv6'), self.int_to_ip(
                int(''.join(ip_address_list)[0:prefix_num] + '1' * zero_count, 2), format='ipv6')
